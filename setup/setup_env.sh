...
# setup_env.sh 现在位于 setup/ 子目录下，但脚本里的路径全部以项目根目录为基准，
# 所以先切到项目根目录再工作。
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" &>/dev/null && pwd)"
PROJECT_ROOT="$(cd -- "$SCRIPT_DIR/.." &>/dev/null && pwd)"
cd "$PROJECT_ROOT"
log_info "工作目录: ${C_BOLD}$PROJECT_ROOT${C_RESET}"
...
# 编译 OpenPCDet 的 CUDA 扩展（.so），但不注册 pcdet.egg-link 到 conda env 里。
# egg-link 会记录绝对路径，跨机器拷贝会失效；我们改由 algos.detectors 运行时
# 把 <repo>/algos/third_party/OpenPCDet 加到 sys.path 来定位 pcdet，路径完全相对。
log_info "编译 OpenPCDet 的 CUDA 扩展 (setup.py build_ext --inplace) ..."
